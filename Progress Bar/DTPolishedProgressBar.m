...
@synthesize progressRect, horizontalMargin, leftToRight, maxValue, currentValue, cornerRadius,
    emptyGradient, barGradient, shadowGradient, highlightColor, numberStyle;

- (id)initWithFrame:(NSRect)frame {
  self = [super initWithFrame:frame];
  if (self) {
    self.shadowGradient = [[NSGradient alloc]
        initWithColorsAndLocations:[NSColor colorWithDeviceWhite:0.3 alpha:1], 0.0,
    ...
    self.emptyGradient = [[NSGradient alloc]
        initWithColorsAndLocations:[NSColor colorWithDeviceWhite:0.25 alpha:1], 0.0,
                                   [NSColor colorWithDeviceWhite:0.45 alpha:1], 1.0, nil];
    self.barGradient = [[NSGradient alloc]
        initWithColorsAndLocations:[NSColor colorWithDeviceWhite:0.7 alpha:1], 0.0,
    ...
    self.numberStyle = @{
      NSFontAttributeName : [NSFont fontWithName:@"Lucida Grande Bold" size:10],
...
- (void)drawRect:(NSRect)rect {
  ...
  if (leftToRight) {
    fillRect.size.width =
        NSWidth(progressRect) * (currentValue + 1) / maxValue + 2 * self.cornerRadius;
  } else {
    fillRect.size.width =
        NSWidth(progressRect) * (currentValue + 1) / maxValue + 2 * self.cornerRadius;
  ...
  NSRect rightStringRect = NSMakeRect(NSMaxX(progressRect) + self.cornerRadius, NSMinY(bounds),
                                      self.horizontalMargin, NSHeight(bounds));
  ...
  NSRect stringRect =
      rectWithSizeCenteredInRect(stringSize, self.leftToRight ? rightStringRect : leftStringRect);
  ...
  stringRect =
      rectWithSizeCenteredInRect(stringSize, self.leftToRight ? leftStringRect : rightStringRect);
...
- (void)setFrameSize:(NSSize)size {
  self.progressRect =
      NSMakeRect(self.cornerRadius + self.horizontalMargin, 0,
                 size.width - 2 * (self.cornerRadius + self.horizontalMargin), size.height);
...
- (void)mouseDragged:(NSEvent *)event {
  ...
  if (NSMouseInRect(cursorPoint, progressRect, [self isFlipped])) {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"SCMouseDragNotification"
                                                        object:self];
  }
}
