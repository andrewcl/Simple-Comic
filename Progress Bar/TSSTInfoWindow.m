...
- (void)resizeToDiameter:(float)diameter {
  NSRect frame = [self frame];
  NSPoint center =
      NSMakePoint(NSMinX(frame) + NSWidth(frame) / 2, NSMinY(frame) + NSHeight(frame) / 2);
...
- (void)drawRect:(NSRect)rect {
  ...
  NSRectFill(bounds);
  if ([self image]) {
    NSGradient* loupeGradient =
        [[NSGradient alloc] initWithStartingColor:[NSColor colorWithCalibratedWhite:0.3 alpha:1]
                                      endingColor:[NSColor colorWithCalibratedWhite:0.6 alpha:1]];

    NSPoint centerPoint = centerPointOfRect(rect);
    [loupeGradient drawFromCenter:centerPoint
                           radius:NSWidth(rect) / 2 - 10
                         toCenter:centerPoint
                           radius:NSWidth(rect) / 2 - 1
                          options:0];
