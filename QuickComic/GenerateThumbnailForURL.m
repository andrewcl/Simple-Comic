...
OSStatus GenerateThumbnailForURL(void *thisInterface, QLThumbnailRequestRef thumbnail, CFURLRef url,
  ...
  NSString *coverName = [UKXattrMetadataStore stringForKey:@"QCCoverName"
                                                    atPath:archivePath
                                              traverseLink:NO];
  //	NSLog(@"page name %@",coverName);
  NSString *coverRectString = [UKXattrMetadataStore stringForKey:@"QCCoverRect"
                                                          atPath:archivePath
                                                    traverseLink:NO];
  ...
  if (![coverName isEqualToString:@""]) {
    //		NSLog(@"has name");
    DTPartialArchiveParser *partialArchive =
        [[DTPartialArchiveParser alloc] initWithPath:archivePath searchString:coverName];
  ...
  if (imageData) {
    ...
    if (CGRectEqualToRect(cropRect, CGRectZero)) {
      //			NSLog(@"no crop");
      canvasRect.size = fitSizeInSize(
          maxSize, CGSizeMake(CGImageGetWidth(currentImage), CGImageGetHeight(currentImage)));
    ...
    CGContextRef cgContext =
        QLThumbnailRequestCreateContext(thumbnail, canvasRect.size, false, NULL);
    if (cgContext) {
      //			NSLog(@"draw");
