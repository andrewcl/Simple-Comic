...
- (void)willTurnIntoFault {
  ...
  if ([[self valueForKey:@"nested"] boolValue]) {
    if (![[NSFileManager defaultManager] removeItemAtPath:[self valueForKey:@"path"]
                                                    error:&error]) {
...
+ (NSArray *)archiveExtensions {
  ...
  if (!extensions) {
    extensions =
        [@[ @"rar", @"cbr", @"zip", @"cbz", @"7z", @"cb7", @"lha", @"lzh", @"tar" ] retain];
  }
...
- (void)nestedArchiveContents {
  ...
  if ([imageArchive isSolid]) {
    do {
      ...
      ++collision;
    } while (![fileManager createDirectoryAtPath:archivePath
  ...
  for (counter = 0; counter < archivedFilesCount; ++counter) {
    ...
    if (!([fileName isEqualToString:@""] ||
          [[[fileName lastPathComponent] substringToIndex:1] isEqualToString:@"."])) {
      ...
      if ([[TSSTPage imageExtensions] containsObject:extension]) {
      ...
      } else if ([[[NSUserDefaults standardUserDefaults] valueForKey:TSSTNestedArchives]
                     boolValue] &&
                 [[TSSTManagedArchive archiveExtensions] containsObject:extension]) {
        ...
        [[NSFileManager defaultManager]
            ...
            withIntermediateDirectories:YES
                             attributes:nil
                                  error:NULL];
        [[NSFileManager defaultManager] createFileAtPath:archivePath
                                                contents:fileData
                                              attributes:nil];
...
- (void)archiveNeedsPassword:(XADArchive *)archive {
  ...
  password = [(SimpleComicAppDelegate *)[NSApp delegate]
      passwordForArchiveWithPath:[self valueForKey:@"path"]];
  [archive setPassword:password];
...
- (id)instance {
  if (!instance) {
    instance = [[PDFDocument alloc]
        initWithData:[NSData dataWithContentsOfFile:[self valueForKey:@"path"]]];
  }
...
- (NSData *)dataForPageIndex:(NSInteger)index {
  ...
  float scale = 1 > (NSHeight(bounds) / NSWidth(bounds)) ? dimension / NSWidth(bounds)
                                                         : dimension / NSHeight(bounds);
...
- (void)pdfContents {
  ...
  for (pageNumber = 0; pageNumber < imageCount; ++pageNumber) {
    imageDescription =
        [NSEntityDescription insertNewObjectForEntityForName:@"Image"
                                      inManagedObjectContext:[self managedObjectContext]];
    [imageDescription setValue:[NSString stringWithFormat:@"%i", pageNumber + 1]
                        forKey:@"imagePath"];
