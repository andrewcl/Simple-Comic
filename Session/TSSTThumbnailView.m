...
- (NSRect)rectForIndex:(NSInteger)index {
  ...
  if ([[[dataSource session] valueForKey:@"pageOrder"] boolValue]) {
    thumbRect =
        NSMakeRect(horGridPos * horSide, NSMaxY(bounds) - side - vertGridPos * side, horSide, side);
  } else {
    thumbRect = NSMakeRect(NSMaxX(bounds) - horSide - horGridPos * horSide,
                           NSMaxY(bounds) - side - vertGridPos * side, horSide, side);
  }
  return thumbRect;
}
...
- (void)buildTrackingRects {
  ...
  for (; counter < ([[pageController content] count]); ++counter) {
    ...
    rectIndex = @(counter);
    tagIndex = [self addTrackingRect:trackRect owner:self userData:rectIndex assumeInside:NO];
...
- (void)zoomThumbnailAtIndex:(NSInteger)index {
  ...
  [(TSSTInfoWindow *)[thumbnailView window]
      setFrame:NSMakeRect(thumbPoint.x - imageSize.width / 2, thumbPoint.y - imageSize.height / 2,
